...
# ssl certification and key file generation
...
# DB tables set-up and configuration
service mysql start
echo "CREATE DATABASE IF NOT EXISTS wordpress;" \
	| mysql -u root --skip-password
echo "CREATE USER IF NOT EXISTS 'jekim'@'localhost' IDENTIFIED BY 'jekim';" \
	| mysql -u root --skip-password
echo "GRANT ALL PRIVILEGES ON wordpress.* TO 'jekim'@'localhost' WITH GRANT OPTION;" \
	| mysql -u root --skip-password
	
# phpMyAdmin set-up and configuration
